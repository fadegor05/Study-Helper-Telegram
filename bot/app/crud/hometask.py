...
async def get_hometasks_all_sorted(telegram_id: int):
    ...
    if tomorrow == 7:
        tomorrow += timedelta(days=1)
    uncompleted = list(hometask_collection.find({'completed_by': {'$nin': [telegram_id]}, '$or': [{'is_hidden': {'$exists': False}}, {'is_hidden': False}]}).sort({'date': -1}))
    uncompleted.sort(key=lambda x: datetime.fromisoformat(x['date']).date() != tomorrow)
    completed = list(hometask_collection.find({'completed_by': {'$in': [telegram_id]}, '$or': [{'is_hidden': {'$exists': False}}, {'is_hidden': False}]}).sort({'date': -1}))
...
async def hide_hometask_by_uuid(hometask_uuid: str):
    connection = await mongo_connection()
    hometask_collection = await mongo_get_collection(connection, 'hometasks')
    hometask_collection.update_one({'uuid': hometask_uuid}, {'$set': {'is_hidden': True}})
