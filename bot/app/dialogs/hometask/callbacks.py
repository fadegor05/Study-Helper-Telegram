...
from app.crud.hometask import change_hometask_status_by_uuid_and_user_id, create_hometask, get_hometask_by_uuid, \
    update_hometask_task_by_uuid, update_hometask_date_by_uuid, hide_hometask_by_uuid
from app.crud.schedule import get_lesson_weekdays_by_uuid
from app.crud.user import is_user_editor_by_telegram_id
from app.dialogs.hometask.states import HometaskInfo, HometaskCreate, HometaskEdit, HometaskDateEdit, HometaskDelete, \
    HometaskMenu
...
async def on_edit_date(c: CallbackQuery, widget: Button, manager: DialogManager):
    hometask_uuid = manager.start_data.get('hometask_uuid')
    await manager.start(HometaskDateEdit.date_hometask, {'hometask_uuid': hometask_uuid})


async def on_chosen_edit_date(c: CallbackQuery, widget: Select, manager: DialogManager, date: str, **kwargs):
    hometask_uuid = manager.start_data.get('hometask_uuid')
...
async def on_chosen_soon_edit_date(c: CallbackQuery, widget: Button, manager: DialogManager):
    hometask_uuid = manager.start_data.get('hometask_uuid')
    hometask = await get_hometask_by_uuid(hometask_uuid)
...
async def on_delete_hometask(c: CallbackQuery, widget: Button, manager: DialogManager):
    hometask_uuid = manager.start_data.get('hometask_uuid')
    await manager.start(HometaskDelete.confirm_hometask, {'hometask_uuid': hometask_uuid})

async def on_delete_confirm_hometask(c: CallbackQuery, widget: Button, manager: DialogManager):
    hometask_uuid = manager.start_data.get('hometask_uuid')
