...
from app.crud.schedule import get_all_schedule, get_all_schedule_sorted, get_day_schedule
...
async def get_schedule_day(dialog_manager: DialogManager, **kwargs):
    ...
    for lesson in schedule_day.get('lessons'):
        if lesson['classroom'] and lesson['building']:
            ...
            lesson.update(place='')
        lesson.update(start_time=datetime.fromisoformat(lesson.get('start_time')).strftime('%H:%M'))
